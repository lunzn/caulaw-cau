...
```
- 输出：知识产权统计（按类型）+ 详细列表
- 示例：`python3 $PI_SKILLS_ROOT/teacher-portal/fetch-patents.py <teacherId>`
- 筛选：`python3 $PI_SKILLS_ROOT/teacher-portal/fetch-patents.py <teacherId> --type=发明专利`
...
- teacherId（可选）：排除自己，传入当前用户的 teacherId
- 示例：`python3 $PI_SKILLS_ROOT/teacher-portal/find-collaborator.py 机器视觉`
- 示例：`python3 $PI_SKILLS_ROOT/teacher-portal/find-collaborator.py 智慧农业 <teacherId>`（排除自己）
